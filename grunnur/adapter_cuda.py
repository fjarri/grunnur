...
class CuContextAdapter(ContextAdapter):
    ...
    def render_prelude(fast_math=False, constant_arrays=None):
        return _PRELUDE.render(fast_math=fast_math, dtypes=dtypes, constant_arrays=constant_arrays)
...
class CuBufferAdapter(BufferAdapter):
    def __init__(
        self, context_adapter, device_idx, size, offset=0, managed=False, ptr=None, base_buffer=None
    ):
    ...
    def set(self, queue_adapter, buf, no_async=False):
        ...
        if isinstance(buf, numpy.ndarray):
            if no_async:
                ...
                pycuda_driver.memcpy_htod_async(ptr, buf, stream=queue_adapter._pycuda_stream)
        else:
