...
def test_platform_take_single(mock_stdin, mock_backend_factory, capsys):
    ...
    devices = check_select_devices(
        mock_stdin, mock_backend_factory, capsys, platforms_devices, inputs=inputs, interactive=True
    )
...
def test_device_take_multiple(mock_stdin, mock_backend_factory, capsys):
    ...
    devices = check_select_devices(
        mock_stdin, mock_backend_factory, capsys, platforms_devices, interactive=True, quantity=3
    )
